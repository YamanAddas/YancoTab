# AGENTS.md — YancoTab Operating Contract
...
3. **File size: 500 lines hard cap, target 400.** Applies to `.js`. New files must land ≤ 500 lines; aim for 400. If a file approaches 500, split before the next addition. Existing offenders (FilesApp 2030, PhotoEditor 1978, MinesweeperApp 1380, ClockApp 1299, SnakeApp 1294, TicTacToeApp 1200, WeatherApp 1136, MemoryApp 1120, SettingsApp 1108, appStorage 913, NotesApp 895, PhotosApp 860, TodoApp 791, and others) are **tech debt**, not a license. Any non-trivial edit to one of them should extract a chunk, not add to it.
...
6. **Games: pure engine + dumb view.** Game logic (state, rules, moves, scoring) lives in a pure module with no DOM refs and has unit tests under `tests/`. The view reads state and renders — it does not mutate game state. See `.Codex/skills/games/SKILL.md`.
...
8. **Yanco design tokens only.** Every color, spacing, radius, and shadow resolves to a variable from `css/tokens.css`. No hardcoded hex in app CSS. Accent is teal (`#00e5c1`, `--accent`). Background is deep space (`#060b14`). See `.Codex/skills/theme/SKILL.md`.
...
- **Per-domain deep context:** `.Codex/skills/<domain>/SKILL.md`
...
- **Git flow:** commit and push directly to `main` whenever a chunk of work lands clean — tests pass, no half-applied edits, no obvious regressions — or whenever Yaman asks. Don't ask permission first. Push command: `git push origin HEAD:main` (local branch name is typically `Codex/...`, so plain `git push` is refused by git's safety rail — this is expected). Never force-push, never skip hooks, never commit secrets, never commit without reading the diff.
